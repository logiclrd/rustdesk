...
#endif
#if HAVE_WAYLAND_POINTER_CONSTRAINTS_UNSTABLE_V1
...
namespace
{
...
#endif
#if HAVE_WAYLAND_POINTER_CONSTRAINTS_UNSTABLE_V1
	struct wp_pointer_constraints_unstable_v1 *wp_pointer_constraints_unstable_v1 = nullptr;
		...
		uint32_t version)
	{
		...
		{
			wayland_seat = wl_registry_bind(registry, id, &wp_pointer_warp_v1_interface, 1);
		...
		{
			wp_pointer_warp_v1 = wl_registry_bind(registry, id, &wp_pointer_warp_v1_interface, 1);
			return;
		}
#endif

#if HAVE_WAYLAND_POINTER_CONSTRAINTS_V1
		if (strcmp(interface, wp_pointer_constraints_unstable_v1_interface.name) == 0)
		{
			wp_pointer_constraints_unstable_v1 = wl_registry_bind(registry, id, &wp_pointer_constraints_unstable_v1_interface, 1);
			return;
		}
#endif
	}
	...
	bool try_pointer_warp_v1(int x, y);
...
# if HAVE_WAYLAND_POINTER_CONSTRAINTS_V1
		if (should_try_pointer_constraints_unstable_v1)
		{
			zwp_locked_pointer_v1 *locked_pointer = zwp_pointer_constraints_v1_lock_pointer(
				wp_pointer_constraints_unstable_v1,
				wayland_surface,
				wayland_pointer,
				nullptr,
